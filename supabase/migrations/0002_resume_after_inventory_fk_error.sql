-- Use this only if 0001_initial_schema.sql stopped at the inventory_movements
-- creation error before it was corrected. It preserves all tables that were
-- successfully created before the error and finishes the initial migration.
...
create index if not exists inventory_movements_item_occurred_at_idx
  on public.inventory_movements(inventory_item_id, occurred_at desc);

create index if not exists company_members_user_id_idx on public.company_members(user_id);
create index if not exists products_company_active_idx on public.products(company_id, is_active, name);
create index if not exists product_variants_company_product_idx on public.product_variants(company_id, product_id, is_active);
create index if not exists customers_company_name_idx on public.customers(company_id, name);
create index if not exists customers_company_phone_idx on public.customers(company_id, phone) where phone is not null;
create index if not exists orders_company_created_at_idx on public.orders(company_id, created_at desc);
create index if not exists orders_company_status_created_at_idx on public.orders(company_id, status, created_at desc);
...
begin
  ...
  if p_discount_cents < 0 then raise exception 'discount cannot be negative'; end if;
  ...
  loop
    ...
    exception when others then raise exception 'invalid order item';
    end;
    if v_quantity is null or v_quantity < 1 then raise exception 'item quantity must be at least one'; end if;
  ...
  update public.orders set subtotal_cents = v_subtotal, total_cents = v_subtotal - p_discount_cents
  where id = v_order_id;
  return v_order_id;
end;
$$;
